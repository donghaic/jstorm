...
public class NimbusServer {
    ...
    private void launchServer(final Map conf, INimbus inimbus) {
        ...
        try {
            ...
            data = createNimbusData(conf, inimbus);  //  nimbus_master上下文对象

            initFollowerThread(conf);  // 监控 /nimbus_master节点，选举leader
    ...
    private void init(Map conf) throws Exception {
        ...
        NimbusUtils.cleanupCorruptTopologies(data); // 删除本地有但ZK中没有的topology代码

        initTopologyAssign();  // 初始化 topology分配线程

        initTopologyStatus();

        initCleaner(conf); // 异步删除无用代码的线程
